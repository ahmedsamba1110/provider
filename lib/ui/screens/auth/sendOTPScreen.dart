      ...
      listener:
        ...
        if (state is VerifyPhoneNumberFromAPISuccess) {
          ...
          if (state.messageCode == '101' && isNewUser) {
            UiUtils.showMessage(
              context,
             message:  'mobileAlreadyRegistered'.translate(context: context),
             type:  ToastificationType.error,
            );
          } else if (state.messageCode == '102' && !isNewUser) {
            UiUtils.showMessage(
              context,
             message:  'mobileNumberIsNotRegistered'.translate(context: context),
             type:  ToastificationType.error,
            );
          } else if (state.messageCode == '103') {
            UiUtils.showMessage(
              context,
             message:  'mobileNumberIsDeactivate'.translate(context: context),
             type:  ToastificationType.error,
            );
          } else {
        ...
        return BlocConsumer<SendVerificationCodeCubit,
          ...
          listener: (
            ...
            if (verifyPhoneNumberState is SendVerificationCodeSuccessState) {
              UiUtils.showMessage(
                context,
               message:  'codeHasBeenSentToYourMobileNumber'.translate(context: context),
              ...
              errorMessage = verifyPhoneNumberState.error
                  ...
                  .translate(context: context);
              UiUtils.showMessage(
                  context,message:  errorMessage,type:  ToastificationType.error);
            }
          },
          builder: (
            BuildContext context,
